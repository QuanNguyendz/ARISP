...
export default function RecruiterCandidatesPage() {
  ...
  const error = actionError || (fetchError ? (fetchError instanceof Error ? fetchError.message : t('loadingError')) : '')
  ...
  const handleInvite = async (e: React.MouseEvent, app: HrApplicationItem) => {
    ...
    try {
      ...
      setActionError(err instanceof Error ? err.message : t('inviteError'))
    } finally {
      setInvitingId(null)
    }
  }
  ...
  return (
    <div className="min-h-screen bg-ink-50 dark:bg-ink-950 p-4 sm:p-6 lg:p-8">
      ...
      {!loading && error && <ErrorAlert message={error} onDismiss={() => setActionError('')} />}
      ...
      {loading && <ApplicantsSkeleton rows={6} />}
