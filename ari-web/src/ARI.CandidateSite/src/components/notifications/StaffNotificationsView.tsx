...
function getFilters(
  ...
  return [
    ...
    { key: 'applicant', label: t('filters.applicant') },
    { key: 'evaluation', label: t('filters.evaluation') },
    { key: 'job', label: t('filters.job') },
  ...
  return (
    ...
    >
        ...
        <div className="mt-1.5 flex items-center gap-3">
          ...
          {n.link && (
            <span className="inline-flex items-center gap-0.5 text-xs font-semibold text-brand-600 dark:text-brand-400">
              {t('actions.viewDetails')} <ChevronRight className="h-3.5 w-3.5" />
            </span>
          )}
        </div>
      </div>
      ...
      <button
        ...
        aria-label={t('actions.deleteNotification')}
...
export default function StaffNotificationsView() {
  const { t } = useTranslation('modules/shared/notifications')
  ...
  useEffect(() => {
    ...
    staffNotificationService
      ...
      .catch(() => active && setError(t('errors.loadFailed')))
  ...
  const clearAll = async () => {
    ...
    if (!window.confirm(t('errors.confirmDeleteAll'))) return
    ...
    try {
      ...
      setError(t('errors.deleteAllFailed'))
    }
  }
    ...
    <div className="min-h-screen bg-ink-50 p-6 dark:bg-ink-950 lg:p-8">
      ...
      <div className="mb-6 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <Trash2 className="h-4 w-4" /> {t('actions.deleteAll')}
          </button>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <>
          ...
          >
            {paged.map((n) => (
              <NotifRow key={n.id} n={n} onOpen={openNotif} onRemove={removeNotif} t={t} />
            ))}
          </motion.div>
