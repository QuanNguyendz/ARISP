...
namespace ARI.Application.Admin.Commands.CreateStaffUser
{
    ...
    /// </summary>
    public record CreateStaffUserCommand(string Email, string FullName, string? Role, string? Department, Guid? ActorId)
    ...
    public class CreateStaffUserCommandHandler : IRequestHandler<CreateStaffUserCommand, Result<CreatedStaffUserDto>>
    {
        ...
        public CreateStaffUserCommandHandler(IUnitOfWork unitOfWork, IPasswordHasher passwordHasher, IEmailService emailService)
        {
        ...
        public async Task<Result<CreatedStaffUserDto>> Handle(CreateStaffUserCommand request, CancellationToken ct)
        {
            ...
            if (normalizedRole != "hr_admin" && normalizedRole != "recruiter")
                return Result.Failure<CreatedStaffUserDto>("Role phải là 'hr_admin' hoặc 'recruiter'.");
            ...
            await AdminSupport.SendStaffWelcomeEmailAsync(_emailService, newUser, tempPassword);

            return Result.Success(new CreatedStaffUserDto(
                newUser.Id, newUser.Email, newUser.FullName, newUser.Role, newUser.Department, newUser.IsActive, newUser.CreatedAt));
        }
    }
}
