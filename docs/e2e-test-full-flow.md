...
| H18 | HM **sửa bộ tiêu chí** sau khi đăng → chấm lại | `hm.dev` → màn tin → thẻ **Bộ tiêu chí chấm CV** → **Sửa** → đổi lời tiêu chí hoặc thêm tiêu chí → **Lưu bộ tiêu chí** | Hộp thoại báo trước *"Lưu thay đổi sẽ ảnh hưởng N hồ sơ…"*. Sau khi lưu, hồ sơ hiện **Đang chấm lại** rồi ra điểm mới. Recruiter chủ tin nhận chuông. Lưu lại y hệt không làm chấm lại. `recruiter.dev` mở thẻ này **chỉ đọc được** |
| H19 | HM **chỉ đổi công thức** → tính lại, KHÔNG gọi AI (ADR-075) | Cùng thẻ trên → đổi **trọng số**, **ngưỡng dải** hoặc **ngưỡng khuyến nghị** ở khối *Công thức chấm* → **Lưu** | Thông báo *"Đã lưu công thức mới… tính lại từ kết quả AI đã có — không chấm lại bằng AI"*. Điểm đổi gần như tức thì; màn hồ sơ ghi *"Tính lại lúc … từ lượt AI đọc CV lúc …"*. Không có lượt gọi Gemini nào (kiểm `prompt_tokens = 0` ở dòng mới) |
| H20 | **Xem trước tác động** trước khi lưu | Trong hộp thoại sửa → **Xem trước tác động** | Bảng từng hồ sơ *điểm/khuyến nghị trước → sau*, số hồ sơ đổi nhãn, số hồ sơ vướng điều kiện, và **số hồ sơ phải nhờ AI chấm lại**. Bấm xem trước **không ghi gì** (điểm trên màn hồ sơ chưa đổi) |
| H21 | **Điều kiện bắt buộc** (knockout) | Thêm tiêu chí, bật **Điều kiện bắt buộc** (vd *"Có chứng chỉ JLPT N2 trở lên"*) → Lưu → chờ AI chấm lại | CV không nêu điều kiện → khối **Điều kiện của công thức** ghi ✗ kèm lý do, khuyến nghị **Chưa phù hợp** nhưng **điểm vẫn hiện** và **trạng thái hồ sơ không đổi**; danh sách ứng viên hiện nhãn *Trượt điều kiện*. CV có chứng chỉ → ✓ kèm trích dẫn từ CV |
| H22 | **Điểm tối thiểu của tiêu chí** | Đặt *Điểm tối thiểu* = 70 cho tiêu chí kinh nghiệm → Lưu | Hồ sơ dưới 70 ở tiêu chí đó → khuyến nghị **Chưa phù hợp** kèm dòng *"Kinh nghiệm: 57 (tối thiểu 70)"*, trong khi *Theo điểm* vẫn là nhãn cũ. Không gọi AI (đây là đổi công thức) |
