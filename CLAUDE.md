...
| ADR-058 | **`booked_count` = số booking `status='scheduled'`** — vị từ chiếm chỗ DUY NHẤT, giống hệt nhau ở `GetSlotsForJobAsync`, `GetInterviewJobsAsync`, SQL đối soát và cờ `OccupiesSeat` (`confirmation_status` KHÔNG tham gia: lịch chờ xác nhận vẫn giữ chỗ — ADR-048). Trước đây màn Phỏng vấn đếm MỌI dòng booking còn server chặn theo cột đã trừ người từ chối → hiện "4/3 ứng viên" trong khi server thấy còn chỗ, modal "Dời lịch" ẩn mất ca dời được. Số hiển thị nay **suy từ dòng booking** (không thể trôi), cột giữ vai trò khoá tương tranh; lệch thì `LogWarning`, KHÔNG tự chữa (endpoint đọc không được ghi). Cột mới **`interview_bookings.declined_by`** (`candidate`/`system`/`staff`) vì `status`+`confirmation_status` không phân biệt được "báo bận" với "hệ thống huỷ vì quá hạn" — giao diện từng phải dò chữ tiếng Việt trong `decline_reason` nên gắn nhãn sai, mà lý do lại là **văn bản ứng viên tự nhập**. **Dời lịch**: thêm `CanManageAsync` + bắt buộc cùng tin (trước đây chỉ so vòng, mà vòng 1 có ở MỌI tin → dời được ứng viên của tin người khác), chiếm chỗ nguyên tử `WHERE booked_count + {n} <= capacity` được-ăn-cả-ngã-về-không, endpoint batch `POST .../bookings/reschedule` thay N request tuần tự. Phủ `CanManageAsync` cho cả 5 endpoint `management/*`; `GetInterviewJobsAsync` lọc chủ tin ở server (nhánh dự phòng cũ ở FE cho recruiter chưa có hồ sơ thấy MỌI tin). **Realtime**: `availability_slots` → chủ tin + `hr_admin`, cố ý KHÔNG gửi ứng viên; `interview_sessions` vào nhóm theo hồ sơ. Gộp 2 trang 1188/1200 dòng → `components/interviews/*` (2 wrapper 5 dòng), ca+ứng viên sang react-query, xoá `InterviewCodePage.tsx` mồ côi, sửa `/recruiter/jobs/:id` vốn là route không tồn tại (404). Migration `ReconcileSlotBookedCount` idempotent (`IS DISTINCT FROM`), **cố ý không đụng `capacity`** — ca vượt sức chứa hiện chip cảnh báo để nhân sự tự quyết |
| ADR-059 | **Duyệt CV = duyệt + xếp lịch vòng 1 trong MỘT thao tác** (`POST /applications/{id}/accept` nhận `slotId` **bắt buộc**): trước đây duyệt xong mà chưa gán ca thì **không thư nào rời hệ thống** — ứng viên chỉ thấy chuông, đúng lỗi người dùng báo. Handler kiểm ca (đúng tin/vòng 1/chưa qua giờ/còn chỗ) **trước** khi duyệt rồi gọi lại `AssignSlotCommand` của ADR-048 (không nhân bản chốt chỗ nguyên tử). **Thư mời gom về `InterviewInviteEmail`**: mở đầu theo vòng (vòng 1 = qua CV; vòng N = đã hoàn thành vòng N-1), theo loại vòng (trắc nghiệm = trực tuyến, bỏ địa điểm + không nhắc phỏng vấn thử), kèm giờ hẹn + địa điểm + 2 nút Xác nhận/Đổi lịch. **Địa điểm là `SystemSetting`** (`interview_location_address|_directions|_map_url`, Super Admin nhập 1 lần — không migration); chưa cấu hình thì **bỏ hẳn dòng địa điểm**. **KHÔNG mở phỏng vấn thử** ở 2 trường hợp — chặn 3 tầng (`PracticeAvailable`, `CheckPracticeEligibilityAsync`, `StartSessionAsync`): vòng trắc nghiệm (cho thử là lộ ngân hàng đề) và vòng **đã lỡ buổi thật** (`HasMissedRealInterviewAsync`: booking `scheduled` qua giờ mà không có phiên `real`); báo bận/hệ thống huỷ KHÔNG tính là lỡ vì nhân sự còn xếp lại. **Tin có vòng trắc nghiệm không rời được bản nháp** khi ngân hàng đề < `OnlineTestQuestionsPerTest` — chặn ở CẢ `draft→pending` lẫn `draft→active` (chỉ chặn cổng gửi duyệt thì publish thẳng là lách được). **Gỡ hẳn đường "mời không kèm lịch"**: nút "Send magic link" thực chất không gửi magic link (token `InterviewInvite` sinh ra rồi không vào link nào, không ai đối chiếu `TokenHash`) → xoá endpoint `send-invite` + command + 5 nút gọi; "Mời hàng loạt" nay mở modal xếp lịch hàng loạt; `SendInterviewInviteAsync`→`OpenRoundForSchedulingAsync` (việc thật: đánh dấu vòng đang hoạt động). Bảng `MagicLink` giữ nguyên cho xác minh email + đặt lại mật khẩu. Thư lên vòng kế bỏ nút "Đặt lịch ngay" (trang tự đặt lịch đã bị ADR-048 gỡ → nút chết). Sửa kèm: 4 thư còn hardcode `localhost:3000`; file locale `vi`/`en` của màn chi tiết ứng viên bị **hoán đổi** + nội suy `{count}` thiếu ngoặc kép nên hiện nguyên chữ. Duyệt hàng loạt dùng 1 ca chung, chạy tuần tự. **Phớt lờ thư mời ≠ báo bận**: bỏ auto-huỷ 48h; dời lịch CHỈ cho người báo bận; người im lặng bị nhắc ở mốc 24h/3h (chuông + email **trả lời đúng luồng thư mời** qua `In-Reply-To`/`References`, lưu `invite_email_message_id`) rồi **quá giờ hẹn + ân hạn 2h mà không có phiên thật → tự đánh trượt** (`not_pass`, `declined_by=system`, trạng thái `no_show`). **Dọn 5 namespace i18n mồ côi**; trong đó lộ ra regression ADR-058: màn Phỏng vấn gộp vào `components/interviews/*` **mất sạch i18n** (87 chuỗi viết cứng dù có nút đổi ngôn ngữ) → dựng `modules/staff/interviews` (125 khoá VI+EN) dùng chung 2 khu vực; `workspaceConfig`/`candidateState` giữ **khoá** thay vì chuỗi |
...
| ADR-060 | **Chấm điểm theo bộ tiêu chí doanh nghiệp**: AI chỉ chấm TỪNG tiêu chí, **backend cộng trung bình có trọng số** rồi so `JobPosting.InterviewPassScore` (mặc định 70) ra verdict — trước đây prompt ép cứng 8 khoá tiếng Anh rồi hỏi luôn `score` tổng, con số đó không phải trung bình có trọng số của gì cả; CV thì rơi về câu viết cứng "Kinh nghiệm 40%, Kỹ năng 40%, Học vấn 20%" vì `ScoringRubric` **không màn nào nhập**. Rubric khai bằng **file Excel theo mẫu**, là 2 loại tài liệu playbook `cv_rubric`/`interview_rubric` (dùng lại phạm vi org/tin/vòng + xoá mềm của ADR-025, khuôn tải-mẫu-rồi-upload của ADR-049); chọn theo **vòng → tin → công ty**. **Tổng trọng số ≠ 100 chặn ngay tại cổng upload** — lọt xuống là mọi điểm sau đều sai mà không có tín hiệu nào. Điểm lưu dạng **ảnh chụp** `{key:{score,label,weight}}` chứ không trỏ rubric sống: HR sửa trọng số về sau thì báo cáo cũ vẫn tự giải thích được. **Tiêu chí AI quên chấm bị loại khỏi CẢ tử lẫn mẫu** (tính 0 là đánh trượt oan vì lỗi model); AI không chấm nổi tiêu chí nào → giữ điểm AI + log. **Chưa khai rubric → giữ nguyên hành vi cũ**. Gemini vẫn chấm CV (ADR-030) nhưng được nhồi rubric + ngữ cảnh playbook. Cạm bẫy đã gặp: 2 chỗ đọc điểm (`PortalSupport`, `EvaluationDtos`) `Deserialize<Dictionary<string,decimal>>` sẽ **ném lỗi rồi trả rỗng** với dạng mới → mất sạch bảng điểm; file mẫu tự sinh **thiếu `CellReference`** nên chính bộ đọc của mình đọc ra bảng rỗng; migration EF sinh default `interview_pass_score = 0` (mọi buổi thành "đạt") và `criterion_scores = ""` (không hợp lệ jsonb) — sửa tay thành 70 và `{}` |
