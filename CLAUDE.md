...
**Hiring Manager của tin** xem report + recording → **Confirm** hoặc **Override** (bắt buộc `override_reason`) kèm thư kết quả qua trình soạn (ADR-074), kèm đề xuất cấp bậc + dải lương để điền sẵn thư mời. Admin chốt thay **luôn** phải nhập `fallback_reason` (≥10 ký tự) → audit log + báo cho HM bị vượt (ADR-068 bỏ ngoại lệ "tin chưa gán HM"). Recruiter không chốt được ở bất kỳ trường hợp nào.

### Phase 6b – Offer → Hired (ADR-061)
Nháp (HM chính/Recruiter **đề xuất** mức lương) → gửi duyệt (báo HR Leader) → **HR Leader chốt** mức lương/điều kiện (ADR-063 — HM không tự chốt đề xuất của chính mình) → gửi ứng viên (qua trình soạn thư) → ứng viên nhận (`hired` → thư xác nhận; đủ `Headcount` thì tin tự đóng — ADR-074) / từ chối (`offer_declined`) / quá hạn (hosted service tự đóng). Hồ sơ sang `offer` lúc **GỬI**, không phải lúc tạo nháp. Một hồ sơ chỉ có **một thư còn hiệu lực** (unique index có filter). Ghi chú đàm phán nội bộ không bao giờ ra Portal.
...
## CV-JD Match Analysis (Gemini AI — ADR-070/071/075)

- **Chỉ chấm khi tin có bộ tiêu chí chấm CV** (HM khai trên phiếu, sửa ở màn tin). Không có → không gọi AI
- **Input:** CV (PDF/DOCX) + file JD gốc + nội dung tin + bộ tiêu chí kèm mức neo. Dự phòng GPT-4o-mini cũng nhận file
- **AI chỉ trả lời định tính, không thấy con số nào**: chọn dải theo lời neo · có/không từng ý kiểm (kèm trích CV) · vị trí 0..1 khi tiêu chí không có ý kiểm · đạt/không đạt cho điều kiện bắt buộc
- **Công thức là của HM, số học là của backend** (ADR-075): điểm trong dải theo trọng số ý → trung bình có trọng số → ngưỡng khuyến nghị của tin; cổng chỉ ép nhãn
- **Reuse:** `cv_jd_analyses` theo khoá (tin, MD5 CV, phiên bản bộ tiêu chí). Đổi lời tiêu chí → AI chấm lại; đổi công thức → **tính lại từ quan sát cũ**; điểm cũ giữ làm lịch sử
- HM/Recruiter/HR xem **phép tính + bằng chứng + cổng** ở màn hồ sơ. Điểm chỉ để tham khảo — ứng viên luôn ứng tuyển được
---
...
16. CV-JD Analysis (Gemini): chạy 1 lần per CV + Job Posting + phiên bản bộ tiêu chí, đính kèm vào Application – không phân tích lại. **Đổi CÔNG THỨC (trọng số, ngưỡng, điểm tối thiểu, trọng số ý) thì tính lại từ quan sát cũ, KHÔNG gọi AI** (ADR-075)
...
28. **Không chấm CV khi tin chưa có bộ tiêu chí chấm CV** (ADR-070) — không có nhánh "AI tự cho điểm"; điểm cuối luôn do backend cộng có trọng số. Mọi cửa ghi bộ tiêu chí đi qua `CvRubricService.SaveForJobAsync`; tin thiếu bộ tiêu chí không sang `pending`/`active` (kể cả admin vượt cổng). Công thức do HM khai (ADR-075): cổng (điều kiện bắt buộc / điểm tối thiểu) chỉ **ép nhãn khuyến nghị**, không bao giờ tự loại hồ sơ
...
| ADR-048 | **Nhân sự gán cứng lịch phỏng vấn** (`POST /api/schedules/assign`), **bỏ hẳn** ứng viên tự chọn; giữ side-effects booking (chốt chỗ nguyên tử, `screening→interview`, gating Interview Code). Ứng viên **xác nhận / báo bận** (`POST /api/candidate/schedule/{bookingId}/confirm` · `/decline`) — báo bận trả chỗ để gán lại qua chính `AssignSlotCommand`. UI: `@ari/shared/ui/AssignSchedulePanel` |
...
| ADR-056 | **Khôi phục FK + UNIQUE + index vận hành vào migration EF** — ADR-055 dựng production từ migration nên mất im lặng lớp schema từng áp tay trên Supabase (kể cả UNIQUE email tài khoản, mã Kiosk). Khai trong `OnModelCreating` (`ConfigureRelationships` + `ConfigureOperationalIndexes`), quan hệ **không navigation property**; **ivfflat → HNSW**; 5 cột cố ý không FK (3 cột đa hình, `account_requests.batch_id`, `questions.playbook_chunk_id`) |
...
| ADR-065 | **Định danh đội/bộ phận của nhân sự; hoàn thiện phiếu.** Bảng `departments`; `User.Department` chuỗi **bỏ hẳn** (nhân viên từng tự sửa được). **KHÔNG lật ADR-061**: `JobAccess` không đọc `DepartmentId` — đây là toàn vẹn dữ liệu, không phải phân quyền. Đội trên phiếu lấy **cứng từ tài khoản**; chỉ Super Admin gán đội; đội giải thể thì **tắt, không xoá**. "Thoả thuận" **suy ra** từ hai ô lương trống. **Không backfill** text cũ |
| ADR-066 | **Thu hồi phê duyệt phiếu — mở lại để sửa, hoặc đóng phiếu.** Hai thao tác một bản chất (huỷ hiệu lực chữ ký): chung hàm kiểm + cờ `CanRevoke`; **không thêm status** (`cancelled` = hết hiệu lực). **Chặn cứng khi phiếu đã dựng thành tin** (TIN là nguồn sự thật). Mở lại thì gỡ phân công; đóng thì giữ dấu vết. Lý do ≥10 vào **cột riêng** `Revoked*` (không ghi đè `Review*`). Chủ phiếu hoặc admin. **Chốt người nhận báo TRƯỚC khi sửa phiếu** |
| ADR-067 | **Phễu duyệt hồ sơ khép kín quanh Hiring Manager; một ca một ứng viên; gỡ Live Avatar.** Recruiter *Duyệt hồ sơ* → `hm_review` (**không báo ứng viên**) → HM duyệt → `screening` = "Chờ xếp lịch" → xếp ca **nằm trọn** trong lịch HM rảnh (bảng `hiring_manager_availabilities`, tin × vòng) → **lúc này** thư "qua vòng" mới gửi. **Ba luật xếp lịch** trong MỘT hàm `ValidateAssignmentAsync`: một ca một ứng viên (theo dòng booking; vòng trắc nghiệm miễn) · không hai buổi chồng giờ · ca trọn trong khung HM. **Phòng chờ**: phiên `real` sinh ở `waiting`, HM *vào phòng* rồi *cho vào* mới `active`; admin mở cửa thay có audit. Chốt giờ với ứng viên (SMS/Zalo) **cố ý ngoài hệ thống**. **Gỡ hẳn HeyGen LiveAvatar** |
...
| ADR-074 | **Hoàn thiện luồng offer.** Thư kết quả vòng (`interview_result`: qua vòng / vòng cuối / cảm ơn — biến thể suy từ CÙNG hàm với trạng thái hồ sơ) **đi kèm lệnh HM chốt qua trình soạn** (`ConfirmReviewRequest.EmailOverride`, ghi `email_logs`); **một báo cáo chỉ chốt một lần** (409). Ứng viên nhận việc → **thư xác nhận** máy gửi (`offer_accepted`, nối luồng thư mời) + **tự đóng tin khi số `hired` ≥ `Headcount` của phiếu** (audit `job_auto_closed_headcount`, báo HM/Recruiter/HR Leader kèm số hồ sơ + thư mời còn mở — không tự loại, không tự thu hồi); màn tin đã đóng liệt kê hồ sơ chưa khép |
| ADR-075 | **Công thức chấm CV do HM quyết định.** HM khai 6 tham số (không phải biểu thức): **điều kiện bắt buộc** (đạt/không đạt, trọng số 0, tối đa 5) · trọng số · **điểm tối thiểu từng tiêu chí** · **trọng số ý kiểm ×1/×2/×3** · **ngưỡng 4 dải** · **ngưỡng khuyến nghị** — mặc định = hành vi cũ (90/70/40 · 80/65/50). Cổng chỉ **ép NHÃN "Reject"** kèm lý do, điểm vẫn tính, `Application.Status` không đổi; "đạt" không trích dẫn → `review`, không ép. Một bộ tính `CvScoreCalculator` (làm tròn **một lần**). **Prompt không còn con số nào** (AI trả `band` + ý kiểm / `position` 0..1 / `met`). **Đổi công thức thì TÍNH LẠI từ quan sát cũ, không gọi AI** (`CvObservationSignature` băm phần AI được hỏi; `derived_from_analysis_id`). `POST .../cv-rubric/preview` xem trước tác động. Màu điểm FE đọc nhãn của server |
