...
- **Cấm `Provider=Local` ở Production + không mặc định âm thầm** (2026-08-06): trước đây `configuration["Storage:Provider"] ?? "Local"` khiến thiếu biến môi trường là rơi về ghi đĩa `./uploads` — mà `docker-compose.prod.yml` đặt `volumes: !reset []` nên thư mục này nằm trong lớp container và **mất sạch mỗi lần deploy**, trong khi upload vẫn trả 200 và vẫn ghi row DB → hỏng hoàn toàn im lặng. Thực tế đã mất CV của **19 hồ sơ** theo cách này. Nay `AddInfrastructure` chỉ nhận `S3`/`Local` tường minh, giá trị lạ → throw, và `Local` + `ASPNETCORE_ENVIRONMENT=Production` → throw ngay lúc boot.
- **Hệ quả khi dev và prod khác provider nhưng chung 1 DB:** máy dev chạy `S3` ghi key `cv/<guid>`, prod chạy `Local` trả key thô → FE ghép thành `https://<origin>/cv/<guid>` → không khớp `/api/`, `/hubs/`, `/uploads/` nên rơi vào fallback SPA và **trả `index.html`**. Triệu chứng nhìn ra ngoài giống hệt lỗi CORS (thông báo của `DocumentViewer` cũng đoán sẵn "có thể do CORS"), rất dễ chẩn đoán nhầm — kiểm tra bằng `GET /cv/<key>` xem có trả `text/html` không trước khi nghi CORS.
...
- **Quyết định:**
  ...
  5. **Trần thời lượng buổi thật** `Interview:RealMaxDurationMinutes` (mặc định 45') dùng chung cơ chế hết giờ của practice (`PracticeTimeoutCloseAsync` nay áp cho cả hai loại phiên, mỗi loại lấy trần riêng) — trước đây real không có trần, phiên treo là chạy vô hạn.
