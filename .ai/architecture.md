...
- **Quyết định:**
  ...
  4. **Hybrid Idle (ADR-011)** áp dụng cho cả 2 mode — tiết kiệm ~90% HeyGen mà UX giữ nguyên (không phải "cắt tech"). **[SỬA bởi ADR-048]** Practice nay **bỏ hẳn avatar** (audio-only) vì rủi ro cạnh tranh concurrency LiveAvatar với buổi thật > lợi ích UX; điểm 3-4 phần avatar-cho-practice không còn áp dụng.
  5. **Trần cứng** cho practice: giới hạn số câu hỏi + thời lượng để tránh đốt token/STT-phút. **[HIỆN THỰC bởi ADR-048]** trần thời lượng 20 phút (`Interview:PracticeMaxDurationMinutes`) — hết giờ AI nói câu kết rồi đóng phiên.
...
### ADR-048: Phỏng vấn thử (Practice) — audio-only, trần 20 phút + AI câu kết, nhập kép voice/keyboard
- **Ngày:** 2026-07-24. **Branch:** `feature/ai/practice-audio-timer-input` (base `origin/develop`).
...
- **Hệ quả / lưu ý:**
  - UI gán nằm ở cột thao tác trang chi tiết ứng viên (HR + Recruiter), qua component chung `@ari/shared/ui/AssignSchedulePanel`: nạp `getSlots(jobPostingId, round)`, lọc slot tương lai còn chỗ, `assign()` xong refetch hồ sơ (`hasScheduledInterview`/`interviewDate`).
  - `useAppNotifications` còn invalidate query key `open-slots` (nay vô hại — không còn ai đặt). Chưa dọn để giảm nhiễu.
  - Chưa có luồng **huỷ/đổi lịch** phía staff — assign chặn nếu vòng đã có booking `scheduled` (phải huỷ thủ công ở DB trước khi gán lại). Là follow-up.
