...
| **Mức neo (Levels)** | Lời mô tả cụ thể cho 4 dải điểm của một tiêu chí (xuất sắc · tốt · đạt một phần · chưa đạt). Người khai chỉ viết LỜI; **AI không bao giờ thấy con số** — để hai lần chấm cùng CV rơi vào cùng dải. Ngưỡng điểm của từng dải là **công thức chấm** của tin (mặc định 90/70/40) |
| **Công thức chấm (CV Scoring Formula)** | Sáu tham số Hiring Manager khai cho từng tin, biến câu trả lời định tính của AI thành điểm (ADR-075): loại tiêu chí · trọng số · **điểm tối thiểu của tiêu chí** · **trọng số ý kiểm** ×1/×2/×3 · **ngưỡng 4 dải** · **ngưỡng khuyến nghị**. Không phải biểu thức tự do — mọi con số vẫn giải thích lại được bằng tay. Đổi công thức thì điểm được **tính lại từ câu trả lời cũ của AI, không gọi AI** |
| **Điều kiện bắt buộc (Knockout)** | Tiêu chí kiểu đạt/không đạt (tối đa 5 mỗi tin, không mang trọng số, không vào trung bình): AI chỉ trả lời có/không kèm trích dẫn CV. Không đạt → **khuyến nghị bị ép "Chưa phù hợp"**, nhưng điểm vẫn tính và **hồ sơ không bao giờ bị loại tự động**. "Đạt" mà không trích được bằng chứng → "chưa xác minh", cần người kiểm |
| **Điểm tối thiểu của tiêu chí (Criterion minimum)** | Ngưỡng tuỳ chọn 1–100 cho một tiêu chí chấm điểm: điểm tiêu chí thấp hơn thì khuyến nghị bị ép "Chưa phù hợp" dù tổng cao — chặn việc bù trừ ở đúng chỗ HM không muốn bù. Khác **điểm sàn** (`InterviewPassScore`, `OnlineTestPassScore`) là ngưỡng đạt của cả buổi |
| **Ngưỡng khuyến nghị (Recommendation tiers)** | Bốn vạch điểm tổng ra nhãn Rất phù hợp / Phù hợp / Cân nhắc / Chưa phù hợp (mặc định 80/65/50), khai theo tin. Màn hình luôn tô màu theo **nhãn server trả về**, không theo ngưỡng riêng của giao diện |
| **Chấm lại (Rescoring)** | Lưu phiên bản bộ tiêu chí mới → mọi hồ sơ của tin tự được chấm lại theo bộ mới (hàng đợi nền). Điểm cũ **giữ làm lịch sử**, không bị xoá; trong lúc chờ, hồ sơ hiện "Đang chấm lại" thay vì điểm cũ. Nếu chỉ **công thức** đổi (AI vẫn được hỏi đúng những câu cũ) thì hệ thống **tính lại từ quan sát cũ, không gọi AI** — nút "Xem trước tác động" cho thấy trước từng hồ sơ sẽ đổi thế nào |
...
| **Match Score** | Điểm phù hợp (0–100) = **trung bình có trọng số** của điểm từng tiêu chí, **do backend tính** theo **công thức chấm của tin** (AI không tự cho điểm tổng, và không thấy con số nào); tiêu chí AI không chấm được bị loại khỏi cả tử lẫn mẫu. HM/Recruiter/HR xem được **phép tính, bằng chứng và các điều kiện bắt buộc** ở màn hồ sơ. Chỉ mang tính tham khảo – candidate luôn có thể ứng tuyển bất kể điểm, và không cổng nào tự loại hồ sơ |
