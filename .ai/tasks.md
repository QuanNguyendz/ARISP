...
- [x] 2026-08-06: **Chẩn đoán lỗi xem CV trên production — production đang chạy `Storage:Provider=Local`.**
  - **Chẩn đoán đầu tiên SAI:** đã kết luận nguyên nhân là bucket R2 thiếu CORS, vì thông báo lỗi của `DocumentViewer` đoán sẵn *"có thể do CORS"*. Đo lại thì không phải.
  - **Bằng chứng prod đang chạy `Local`:** `GetJobByIdQuery` có gọi `_fileStorage.GetUrlAsync`, nhưng prod trả về **key thô** (`cv/<guid>.pdf`, `/uploads/<guid>.pdf`). Chỉ `LocalFileStorageService.GetUrlAsync` trả nguyên key; `S3FileStorageService` sẽ ký thành `https://…r2.cloudflarestorage.com/…`.
  - **Cơ chế hỏng thật:** CV có key `cv/<guid>.docx` (do máy dev chạy docker với `Provider=S3` ghi lên R2) + prod trả key thô → FE ghép thành `https://staff.arisp.io.vn/cv/<guid>.docx` → không khớp `/api/`, `/hubs/`, `/uploads/` → rơi vào fallback SPA → **trả `index.html` (`text/html`)**. `docx-preview` nhận HTML nên ném lỗi. Đo trực tiếp: `GET /cv/<key>` trên **cả 2 origin** đều trả `200 text/html`.
  - **Mất dữ liệu đã xảy ra:** 19 hồ sơ có key `/uploads/…` (prod ghi vào đĩa trong container, mà `docker-compose.prod.yml` đặt `volumes: !reset []` nên mất sạch mỗi lần deploy). Kiểm tra `GET /uploads/<key>` → **404**. Đối chiếu `ari-service/src/ARI.API/uploads` trên máy dev: chỉ trùng 2 tên và cả 2 đều **0 KB** → không khôi phục được.
  - **Chặn tái diễn:** `DependencyInjection` bỏ mặc định âm thầm `?? "Local"` → chỉ nhận `S3`/`Local` tường minh, giá trị lạ thì throw, và **cấm `Local` khi `ASPNETCORE_ENVIRONMENT=Production`** (kèm thông điệp chỉ rõ file sẽ mất khi deploy). Guard này đúng ra đã bắt được sự cố ngay từ lần boot đầu.
  - **Thứ tự bắt buộc:** phải thêm `Storage__Provider=S3` + `Storage__S3__*` vào `docker/.env` **trên VPS trước**, rồi mới deploy guard — deploy trước thì backend prod sẽ crash lúc boot.
  - CORS rule trên R2 (đã đặt, xác minh bằng preflight: đúng 2 origin, origin lạ bị 403) **vẫn cần** — là điều kiện đủ cho bước chuyển prod sang S3, chỉ là không phải nguyên nhân của triệu chứng này. Tài liệu: [docs/r2-storage-cors-setup.md](../docs/r2-storage-cors-setup.md) + ghi chú ADR-036.
