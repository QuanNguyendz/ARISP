...
- [x] 2026-07-03: **Gộp gợi ý CV vào bộ lọc — bỏ section "Việc phù hợp với bạn" riêng, dùng sort "Độ phù hợp (theo CV)" — BE + FE.**
  - Theo yêu cầu user (cho gọn): **xoá hẳn** section "Việc phù hợp với bạn" đứng riêng trên `FindJobPage`; toàn bộ chức năng khớp CV giờ nằm ở option sort **"Độ phù hợp (theo CV)"** (đã có). Badge "Khớp N kỹ năng" vẫn giữ: tính **client-side** ngay trên thẻ danh sách chính khi `sortBy==='relevance'` (`job.skills ∩ profileSkills`, không cần thêm dữ liệu BE).
  - Dọn code chết của tính năng "recommended" cũ (giờ thừa vì relevance sort thay thế): FE bỏ state `recommended` + fetch + `jobService.getRecommendedJobs` + type `RecommendedJob`; BE bỏ endpoint `GET /api/portal/jobs/recommended` (`CandidatePortalController`) + DTO `RecommendedJobResponse`. FE `tsc`: exit 0; BE build: 0 error.
- [x] 2026-07-03: **Job Board ứng viên: sort "Độ phù hợp (theo CV)" + lọc theo cấp bậc cạnh dropdown sort — BE + FE.**
  - BE (`JobsController.GetJobs`, `GET /jobs`, AllowAnonymous): thêm `sortBy=relevance`. Khi caller là ứng viên đăng nhập có kỹ năng → helper `GetCurrentCandidateSkillsLowerAsync` đọc `CandidateAccount.SkillsJson` (theo claim `sub`/NameIdentifier); items query bỏ sort/phân trang ở SQL, chấm điểm + xếp theo số kỹ năng trùng desc → tin mới nhất rồi phân trang **ở bộ nhớ** (tránh sort collection ở SQL). Không đăng nhập / staff / hồ sơ trống → tự rơi về "mới nhất". `totalCount` vẫn từ count query (đúng phân trang). Đây là **sort** (mọi tin vẫn hiện, khớp nhiều lên trước), khác với section "Việc phù hợp" (chỉ tin khớp). Build API: 0 error.
  - FE (`FindJobPage`): dropdown sort thêm option **"Độ phù hợp (theo CV)"** (`sortBy='relevance'`, chỉ hiện khi `profileSkills.length > 0`); thêm 1 **select "Mọi cấp bậc"** cạnh dropdown sort lọc theo `facets.experienceLevels` (đồng bộ `filters.experienceLevels`, single-select). Effect reset `relevance→newest` khi mất kỹ năng (đăng xuất/xoá skill). `apiClient` tự đính token nên `/jobs` mang JWT ứng viên để BE chấm điểm. `tsc --noEmit`: exit 0.
  - Làm rõ với user: "vị trí" = **cấp bậc/kinh nghiệm** (không phải địa điểm — địa điểm đã có sẵn); đặt cả hai control cạnh dropdown sort hiện tại.
- [x] 2026-07-03: **Job Board ứng viên: thẻ gợi ý CV về 1 cột (`space-y-4`) cho đồng nhất với danh sách chính — FE.**
