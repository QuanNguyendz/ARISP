...
- [x] 2026-07-14: **Fix STT mất transcript lần 2 (cờ aiSpeaking kẹt do avatar event) + xác định avatar mất hình = HẾT CREDIT HeyGen free — kiểm chứng bằng E2E Playwright thật.**
  - **Avatar "lúc hiện lúc không" rồi mất hẳn = GÓI FREE LiveAvatar:** free = 2 phút/phiên, tổng 10 phút LITE (đã cạn — API trả `No credits available for start session`). Cần Essential $19/th (phiên 5 phút) hoặc Standard $99/th (phiên 20 phút, đủ 1 buổi liền mạch). Deepgram không phải vấn đề gói ($200 free credit, không giới hạn phiên).
  - **STT mất transcript:** avatar phát trễ vài giây → watchdog (duration+3s) nhả cờ sớm → `AVATAR_SPEAK_STARTED` bắn trễ set cờ lại KHÔNG kèm watchdog → avatar chết giữa câu (cap 2 phút free) nên `SPEAK_ENDED` không bắn → cờ kẹt true vĩnh viễn → mọi transcript bị vứt. Fix: `speakUntilRef` (hạn dùng lượt nói) — mọi đường set cờ đều kèm watchdog (trần cứng 90s); `SPEAK_STARTED` ngoài cửa sổ +10s = sự kiện lạc → bỏ; `SPEAK_ENDED` chỉ xử lý khi đang nói (tránh xóa oan buffer); `SESSION_DISCONNECTED` giữa lượt nói → nhả cờ ngay; đệm avatar 3s→6s; log `[speak]`/`[media-config]`/`[deepgram] INIT|RECORDING` chẩn đoán.
  - **E2E Playwright tự động (scratchpad practice-e2e.js):** login API → bơm `arisp-auth` localStorage → override `getUserMedia` (audio = MP3 tiếng Việt edge-tts loop qua WebAudio, video = canvas) → DeviceCheck → phòng phỏng vấn. Kết quả 4/4 PASS: MediaRecorder ghi; câu hỏi AI hiện (rag-service + GPT-4o thật); cờ speaking nhả; **transcript ứng viên hiện đúng nguyên văn audio tiếng Việt**. Avatar fallback WebAudio hoạt động đúng thiết kế khi hết credit. Test data: account `e2e-stt-test@arisp.dev` + application `fc6732b5` (job Fullstack Cloud) — tái dùng được cho lần test sau.

- [x] 2026-07-13: **Giới hạn lượt phỏng vấn thử thành config `Interview:PracticeAttemptsPerRound` — dev test lặp lại không vướng gating.**
  - `InterviewOptions` (ARISP.Application.Options) bind từ section "Interview" (mirror pattern MediaOptions); mặc định **1 lượt/vòng** (ADR-038 giữ nguyên cho prod). `<= 0` = không giới hạn — đã đặt `0` trong `appsettings.Development.json` (+ cập nhật file .example kèm ghi chú đừng bật ở prod).
  - Áp cả 2 chốt: `InterviewService.StartSessionAsync` (chặn cứng, đếm số phiên practice/vòng so với limit) + `CandidatePortalController` (`PracticeAvailable` — nút "Bắt đầu phỏng vấn thử" hiện lại sau mỗi lượt khi unlimited). Options inject dạng optional (ctor default) — test/DI cũ không vỡ.

- [x] 2026-07-13: **Xoá màn "Kết quả" cũ (FeedbackPage mock) — kết quả phỏng vấn hợp nhất vào chi tiết hồ sơ ứng tuyển.**
  - Màn kết quả mới ĐÃ có sẵn: `ApplicationDetailPage` (`/candidate/applications/:id`, light style, dữ liệu thật per-round + nhận xét HR). `FeedbackPage` (`/candidate/results`, dark style, dữ liệu hardcode) bị xoá.
  - Nút "Xem kết quả" sau phỏng vấn thử → `/candidate/applications/:applicationId` (đúng hồ sơ vừa phỏng vấn); `InterviewRoomPage` (mock) → danh sách hồ sơ. Route `/candidate/results(/:id)` redirect về `/candidate/applications`. Menu user header đổi thành "Lịch phỏng vấn" (`/candidate/interviews`); footer link → Hồ sơ ứng tuyển; bỏ helper `candidateResult`, thêm `candidateApplicationDetail`.

- [x] 2026-07-13: **Practice Interview đợt 2: UI scroll + avatar reconnect + kiểm soát ngôn ngữ (ADR-018) + AI tự kết thúc kèm lời cảm ơn.**
  - **UI:** phòng phỏng vấn chuyển `min-h-screen` → `h-screen overflow-hidden` — page KHÔNG giãn theo transcript; panel Transcript `min-h-0 + overflow-y-auto` cuộn nội bộ + auto-scroll xuống cuối khi có message/draft mới; mobile giữ layout 2 hàng `grid-rows-[minmax(45vh,1fr)_minmax(0,1fr)]`.
  - **Avatar rớt giữa phiên** (LiveAvatar LITE idle-timeout/mạng → "mất hình AI"): `SESSION_DISCONNECTED` → mint token MỚI qua media-config → dựng lại `LiveAvatarSession`, tối đa 2 lần (reset quota khi stream sống lại); trong lúc chờ tự rơi xuống WebAudio.
  - **Kiểm soát ngôn ngữ:** .NET `QuestionContext/SessionContext` thêm `Language` (trước đây rag-service có field nhưng .NET KHÔNG gửi → prompt luôn mặc định tiếng Việt) — truyền `session.InterviewLanguage`. Prompt next-question thêm LANGUAGE RULE: ứng viên trả lời sai ngôn ngữ → AI mở đầu bằng nhắc nhở ngắn rồi hỏi tiếp. `LanguageAssessment` thêm `language_adherence` (nhận xét tuân thủ ngôn ngữ) — lưu vào `evaluations.language_assessment` (đổi sang JsonSerializer, hết handcraft JSON); evaluate prompt yêu cầu nêu rõ trong reasoning nếu ứng viên nói sai ngôn ngữ nhiều lần.
  - **AI tự kết thúc + farewell:** prompt có ENDING RULE (chỉ khi ≥5 câu + hết must-ask): đủ độ bao phủ JD/ứng viên bỏ cuộc → output `[END_INTERVIEW] <lời cảm ơn>`; cap 12 câu đổi từ cắt phụt sang `ForceClosing=true` (AI chỉ sinh farewell). `InterviewService` nhận diện marker → publish `ReceiveClosing` (text) + `ReceiveClosingAudio` (TTS) → `EndSessionAsync` (evaluation như cũ). FE: hiện farewell trong transcript, phát audio (fallback browser TTS sau 6s), server báo completed khi đang nói → CHỜ nói xong mới chuyển màn kết thúc (chốt chặn 20s). `OpenAIProvider` in-process đồng bộ cùng hành vi. Smoke test prompt (7 case) pass; BE Infrastructure build 0 lỗi; FE tsc pass.

- [x] 2026-07-13: **Practice Interview: fix STT không nhận giọng (MediaRecorder chết vì video track) + watchdog cờ aiSpeaking + chuyển sang gửi trả lời thủ công.**
  - **Root cause "nói không hiện transcript" (kiểm chứng bằng Playwright + Chrome fake media):** `MediaRecorder` với mimeType `audio/webm` KHÔNG chấp nhận stream chứa video track — DeviceCheck trả stream cam+mic nguyên vẹn → `NotSupportedError` ngay tại `start()` → Deepgram không nhận audio → CLOSE 1011 → vòng reconnect vô hạn (mỗi lần OPEN reset retry counter). Fix: **tách audio-only `new MediaStream(getAudioTracks())`** trước khi ghi; lỗi khởi tạo recorder giờ coi là fatal cục bộ (set retry = MAX + đóng socket, không spin). Test repro/fix: full stream → NotSupportedError y hệt; audio-only → 5 chunks/19.5KB trong 1.2s.
  - **Phòng ngừa thêm — speak watchdog:** guard chống echo bỏ transcript khi `aiSpeakingRef=true` nhưng HeyGen có khi không bắn `AVATAR_SPEAK_ENDED` → cờ kẹt nuốt transcript. Mỗi lần phát audio tự tính thời lượng (PCM 24k: bytes/2/24000; WebAudio: `buffer.duration`; browser TTS: ước lượng theo text) + đệm 3–4s, quá hạn tự nhả + log `[speak-watchdog]`. Avatar path tự `setSpeaking(true)` khi `repeatAudio`.
  - **UX gửi trả lời thủ công (theo yêu cầu):** bỏ auto-submit khi Deepgram `UtteranceEnd`; transcript đã chốt (`is_final`) tích lũy vào `draft` hiển thị live ở panel Transcript (bubble "Bạn — đang trả lời" + interim italic); ứng viên bấm **"Gửi trả lời"** (nút đổi gradient khi có nội dung, disabled khi rỗng) mới invoke `SubmitAnswerText` → AI sinh câu hỏi kế.
  - **Chuẩn hóa type:** payload SignalR (`QuestionPayload`/`QuestionAudioPayload`/`SessionStatusPayload`) + message Deepgram hết `any`.
  - **Vận hành:** lỗi `StartInterview` connection refused localhost:8000 do **rag-service không chạy** (venv hỏng — chỉ còn `Lib/`, mất `Scripts/`) → tạo lại venv Python 3.12 + `pip install -e .`; lưu ý WinError 10013 khi port 8000 đã bị chiếm. Backend từng crash `DirectoryNotFoundException wwwroot` → tạo `wwwroot/.gitkeep`.
