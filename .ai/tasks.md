...
- [x] 2026-08-11: **Unit test gửi lời mời phỏng vấn (test-plan B20) — 3 test mới, tổng 525/525 pass.** File `ApplicationFlow/SendInterviewInviteCommandHandlerTests.cs` phủ `SendInterviewInviteCommandHandler` (handler mỏng delegate `IApplicationService.SendInterviewInviteAsync`): config `Frontend:CandidateBaseUrl` + RoundNumber=2 → gọi service đúng appId + baseUrl + round 2, trả kết quả nguyên; thiếu `CandidateBaseUrl` → fallback `Authentication:AdminFrontendUrl`; thiếu cả 2 config + service fail → baseUrl fallback `http://localhost:3000` + propagate Failure nguyên message. Fake `RecordingApplicationService` (local, ghi lại appId/baseUrl/round) + config qua `ConfigurationBuilder().AddInMemoryCollection`. Lưu ý clash: `Common` là namespace test → field default phải fully-qualify `ARI.Application.Common.Result`. `dotnet test`: **525/525 pass**.

- [x] 2026-08-11: **Unit test Interview management (test-plan B15–B19) — 16 test mới, tổng 522/522 pass.** **B15** `Interviews/SendBookingReminderTests` (thiếu booking/hồ sơ/ca → lỗi + `Reminder24hSent=false`; happy → DB Notification `schedule_reminder` + realtime `ReceiveUserNotification` + `Reminder24hSent=true`; không account → vẫn đánh dấu, không bell). **B16** (bổ sung vào `InterviewCodes/GenerateInterviewCodeTests`): null list → lỗi "không được để trống"; 2 hồ sơ đã đặt lịch vòng 2 → 2 mã `RoundNumber=2` khác nhau + 2 audit `interview_code_generated`. **B17** `GetInterviewJobsTests` (không job → rỗng; active + deadline quá khứ + 2 slot R1/R2 + 2 booking (1 confirmed) + 1 session completed → JobStatus `closed`/TotalSlots 2/Booked 2/Confirmed 1/MaxRound 2/Sessions 1/Completed 1/NextSlotTime = ca tương lai). **B18** `GetCandidatesInSlotTests` (hồ sơ `not_pass` → override Confirmation `declined`/DeclineReason "Đã bị loại…"/BookingStatus `cancelled`; hồ sơ active → map session THẬT + Eval điểm 72.6→73 (làm tròn) + mã On-site còn hiệu lực, **loại phiên practice**). **B19** `GetMediaConfigTests` (thiếu session/hồ sơ → lỗi; non-owner không kiosk → Forbidden; **kiosk real** → Success bypass IDOR, MaxDuration 2700 (45'), Deepgram/HeyGen **null** do provider throw bị nuốt; **practice** `PracticeUseAvatar=false` → HeyGen null, MaxDuration 1200 (20')). Dùng `InterviewServiceFactory.Create(uow, notif)` 2-arg (providers all-throwing = đúng nhánh best-effort của GetMediaConfig; `_interviewOptions ?? new()` cho trần 45'/20' mặc định); tái dùng `SchedulingData`/`ApplicationData`/`InMemoryUnitOfWork`. `dotnet test`: **522/522 pass**.

- [x] 2026-08-11: **Unit test Admin lifecycle (test-plan B14) — 18 test mới, tổng 506/506 pass.** 4 handler: **CreateStaffUser** (`Admin/CreateStaffUserCommandHandlerTests`) role không hợp lệ (`super_admin`/null) → lỗi không tạo/audit/email; email trùng ` A@X.com ` normalize → Conflict; FullName rỗng → short-circuit; happy `HR_Admin` → user email lower/role lower/trim/IsActive/PasswordHash `hashed:` (≠ plaintext) + AuditLog `staff_account_created` + SaveChanges 1 + welcome email + DTO echo. **ApproveAccountRequest** (request lạ → NotFound; đã xử lý → lỗi; email đã có user → Conflict giữ pending; happy → tạo user normalize + req approved/ReviewedBy/At/CreatedUserId + audit `account_request_approved` + email + realtime `ReceiveAccountRequestUpdate` cho leader). **DeactivateUser** (tự khóa → chặn TRƯỚC khi load; reason null/whitespace → lỗi; đã khóa → lỗi; happy → IsActive=false + LockReason trim + audit `user_deactivated` metadata chứa reason/email + SaveChanges 1). **UploadPlaybook** (`Playbooks/UploadPlaybookCommandHandlerTests`, ADR-039: parser throw → không lưu/không ingest; happy `.pdf`/scope `job` → PlaybookDocument `ready`/FileFormat `pdf`/DocumentType trim + Ingest 1 lần playbook/scope/type; parser rỗng → lưu nhưng bỏ ingest; ingest throw → ServerError + `DeleteAsync` file bù trừ). Tái dùng `FakePasswordHasher` (Auth), `RecordingEmailService`/`RecordingNotificationService`/`RecordingFileStorage`/`StubDocumentParser` (TestSupport); thêm `ThrowOnIngest` + mở rộng tuple `Ingested` (thêm Scope/DocumentType) cho `RecordingRagIngestionService` (không phá 2 consumer cũ chỉ đọc SourceType/SourceId/Text). `dotnet test`: **506/506 pass**.

- [x] 2026-08-11: **Unit test Auth core (test-plan B13) — 27 test mới, tổng 488/488 pass.** 5 file + fakes trong `tests/ARI.Application.UnitTests/Auth/`: **VerifyMagicLink** (email lạ → not_found không mint; candidate có → JWT cấu hình, mint 1 lần; ⚠ token rác/hết hạn VẪN Success — ghi nhận gap chủ ý không kiểm token; email `  USER@X.com ` normalize khớp). **CandidateLogin** (không account → InvalidCredentials, không Verify; PasswordHash rỗng → PasswordlessGoogle; EmailVerified=false + mật khẩu đúng → EmailNotVerified, không token/refresh; happy → LastLoginAt + access/refresh + Role Candidate + FullName fallback "Candidate"). **StaffLogin** (email chưa provision → InvalidCredentials không tạo draft; IsActive=false → AccountDisabled kiểm trước Verify; PasswordHash null → SsoOnly; Verify false → InvalidCredentials, true → LastLoginAt + JWT staff + refresh + Role=user.Role). **RegisterCandidate** (email trùng → "Email already registered" không lưu/không email; mật khẩu yếu Theory 4 case theo `IsStrongPassword`; happy → email lower+trim, EmailVerified=false, PasswordHash `hashed:…`, MagicLink audience `candidate_verify` TTL~24h + 1 email; trùng email + mật khẩu yếu → lỗi trùng thắng). **VerifyCandidateEmail** (không candidate → lỗi; đã verify → idempotent; link hợp lệ → EmailVerified=true + UsedAt one-time; Theory hết hạn/đã dùng/sai audience/sai hash → từ chối giữ false). Fakes mới `AuthTestSupport.cs`: `FakeTokenService` (đếm mint), `FakePasswordHasher` (VerifyResult + đếm), `RecordingEmailQueue`, factory `AuthData` (Candidate/Staff/VerifyLink + EmptyConfig). `dotnet test`: **488/488 pass**.

- [x] 2026-08-11: **Unit test Candidate Portal apply/verify/evaluation (test-plan B10–B12) — 18 test mới, tổng 461/461 pass.** 3 file trong `tests/ARI.Application.UnitTests/CandidatePortal/`: **B10** `ApplyToJobCommandHandlerTests` (ứng tuyển trùng → `AlreadyApplied=true` + `ExistingApplicationId`, không lưu CV/không gọi service; happy path CV đính kèm → lưu 1 file MIME `application/pdf`, ráp `SubmitApplicationRequest` source `job_board` + hash MD5 + CvText parse + trim tên/điện thoại/notice; không đính kèm & hồ sơ thiếu CV → code `no_cv`; service fail → CV đã lưu bị `DeleteAsync` bù trừ + lỗi propagate). **B11** `VerifyCvInfoCommandHandlerTests` (so khớp CODE 0 token AI: tên bỏ dấu/thường theo token bỏ token 1 ký tự + 9 số cuối điện thoại; thiếu tên → chỉ bullet "Họ và tên"; điện thoại khác → chỉ "Số điện thoại"; điện thoại <8 số bỏ qua; CV rỗng → `IsMatch=true` guard; acc null → Unauthorized; thiếu CV → `no_cv`; CV đọc rỗng → `cv_unreadable`). **B12** `GetMyEvaluationQueryHandlerTests` (IDOR: account+email lệch, hồ sơ đã gắn tài khoản → Forbidden "Forbidden" không lộ eval; owner nhưng `ShareEvaluation=false` → "chưa được chia sẻ" ≠ NotFound; owner + share → trả đủ AiVerdict/OverallScore/CriterionScores/QuestionAnalyses/LanguageAssessment; thiếu phiên/hồ sơ/đánh giá → NotFound). Tái dùng `FakeApplicationService` (JobBoard), `RecordingFileStorage`+`StubDocumentParser` (TestSupport), `ApplicationData`; assert anonymous object của B12 qua `JsonSerializer.Serialize(value, value.GetType())` (runtime type). Lưu ý clash: `HrReview` là namespace test → phải fully-qualify `ARI.Domain.Entities.HrReview`. `dotnet test`: **461/461 pass**.

- [x] 2026-08-11: **Unit test lưu video phỏng vấn thật (test-plan B2) — 6 test mới, tổng 443/443 pass.** File `tests/ARI.Application.UnitTests/Interviews/SaveRecordingTests.cs` phủ `InterviewService.SaveRecordingAsync` (ADR-052): phiên lạ → "Không tìm thấy phiên phỏng vấn"; buổi THỬ → "Phỏng vấn thử không quay video" (ADR-038.6), storage rỗng, `RecordingUrl` null, SaveChanges==0; content rỗng → "Dữ liệu ghi hình rỗng"; `MaxRecordingSizeMb=1` + 2MB → "vượt quá 1MB" (không lưu, `RecordingUrl` null). Happy path (options mặc định 300MB/7 ngày): content-type `"video/webm;codecs=vp9,opus"` → storage nhận `"video/webm"` (bỏ tham số codecs), `RecordingUrl="stored/kiosk.webm"`, `RecordingSizeBytes=len`, `RecordingExpiresAt≈now+7d`, `RecordingDeletedAt=null`, response `Saved=true`; upload lại (đã có `RecordingUrl` cũ) → `DeleteAsync` key cũ TRƯỚC + trỏ bản mới + reset `RecordingDeletedAt`. Thêm overload `InterviewServiceFactory.Create(uow, notif, IFileStorageService, InterviewOptions?)` cắm `RecordingFileStorage` thật + `InterviewOptions` (không đụng 2 overload cũ nhờ khác kiểu tham số thứ 3). `dotnet test`: **443/443 pass**.

- [x] 2026-08-11: **Unit test Online Test staff (test-plan B5–B9) — 24 test mới, tổng 437/437 pass.** 5 file trong `tests/ARI.Application.UnitTests/OnlineTest/`: **B5** `UpdateOnlineTestQuestionCommandHandlerTests` (validate TRƯỚC GetById → single mà 2 đáp án ra "đúng 1 đáp án"/SaveChanges==0; recruiter khác chủ tin → Forbidden, entity không đổi; chủ tin sửa `"  Edited?  "`+multiple `{2,0,2}` → trim + `[0,2]` + `CorrectOption=0` + `UpdatedAt`). **B6** `UpdateOnlineTestSettingsCommandHandlerTests` (biên điểm sàn 0–100 / số câu 1–200 / thời lượng 1–300' kiểm trước lookup, Theory 6 case out-of-range; boundary (100,200,300)+(0,1,1) lưu 3 field + DTO; recruiter khác → Forbidden). **B7** `GetOnlineTestResultForStaffQueryHandlerTests` (chưa thi → Success **null** ≠ NotFound; 2 lượt → lấy MỚI NHẤT theo `CreatedAt`, `PassScore` từ job; app lạ → NotFound; non-owner → Forbidden). **B8** `GetOnlineTestResultsByJobQueryHandlerTests` (non-owner → Forbidden; 3 sub (90/50/70, pass70) → TotalQuestions=3(bank)/SubmissionCount=3/Passed=2/Avg=70.0/High90/Low50/Rows sort điểm giảm dần; 0 sub → stats=0, Rows rỗng, bankCount giữ). **B9** `ImportOnlineTestQuestionsCommandHandlerTests` (non-owner→Forbidden & job lạ→NotFound **không đọc file**; bytes rác → "Không đọc được file Excel"/SaveChanges==0; xlsx thật header+remap qua cột trống → Imported=2, single `[1]` / multiple `[0,1]`; import từng phần header+1 hợp lệ+3 lỗi+dòng trống → Imported=1/Failed=3/Errors dòng {3,4,5}/SaveChanges==1). Thêm helper `OnlineTestData.Submission(...)` + `BuildXlsx(params string[][])` (dựng .xlsx thật bằng OpenXML — cell có `CellReference` để `ReadRows` nhận cột, ô rỗng thưa). Tái dùng `InMemoryUnitOfWork` + `OnlineTestData`, `DocumentFormat.OpenXml` chảy transitively từ ARI.Application. `dotnet test`: **437/437 pass**.

- [x] 2026-08-11: **Unit test ghi nhận tín hiệu nghi vấn (test-plan B3) — 8 test mới, tổng 413/413 pass.** File `tests/ARI.Application.UnitTests/Interviews/RecordCheatSignalTests.cs` phủ `InterviewService.RecordCheatSignalAsync` (nguồn gọi `SessionHub.ReportCheatSignal` + `InterviewSessionFeature`, ADR-054): guard loại tín hiệu rỗng/khoảng trắng → "Thiếu loại tín hiệu" và chạy TRƯỚC khi tra phiên (uow rỗng vẫn ra đúng lỗi này, không phải "không tìm thấy phiên"), không lưu + `SaveChangesCount==0`; phiên lạ → "Không tìm thấy phiên phỏng vấn"; chuẩn hoá loại `"  FULLSCREEN_EXIT "` → `fullscreen_exit` (trim+lower) và trả về **số tín hiệu cùng loại** của phiên (seed sẵn 1 → trả 2), loại khác không tính vào count; làm sạch payload: sai prefix (`DROP TABLE users`) và quá dài (>2000 ký tự) → ép `"{}"` bảo vệ cột jsonb, JSON hợp lệ `{"x":1}` chỉ trim giữ nguyên, mảng `[1,2]` cũng hợp lệ. Tái dùng `InMemoryUnitOfWork`/`RecordingNotificationService`/`PracticeData` + overload `InterviewServiceFactory.Create(uow, notif)` (RecordCheatSignal chỉ chạm UnitOfWork, không cần AI/TTS/storage). `dotnet test`: **413/413 pass**.

- [x] 2026-08-09: **Xem JD ở chi tiết tin (staff) hiển thị bản đã đóng dấu duyệt khi tin đã duyệt.** Trước đây nút xem JD chỉ mở file gốc (`jdFileUrl`, chưa có chữ ký số) còn bản đóng dấu (`signedJdFileUrl`, do `JdStampService` sinh khi duyệt) nằm ở nút xanh riêng. Nay `hr/JobPostingDetailPage.tsx`: nút JD mở `signedJdFileUrl || jdFileUrl` (đã duyệt → xem bản có con dấu ĐÃ DUYỆT ngay), thêm badge "Đã đóng dấu"/"Signed" + đổi màu emerald khi có bản ký, **bỏ nút xanh trùng lặp**; `recruiter/JobDetailPage.tsx` cũng ưu tiên bản ký. i18n `jdSection.signedBadge` (VI/EN); gỡ import `Download` thừa. Typecheck StaffSite pass. *(Ghi chú: con dấu vẫn ghi "HR LEADER" trong `JdStampService.cs` — chưa đổi theo rename HR Admin.)*

- [x] 2026-08-09: **Header staff (HR + Recruiter): dropdown tìm kiếm tổng hợp Tin tuyển dụng + Ứng viên.** Thay ô search điều hướng-1-đích bằng component chung `ARI.StaffSite/src/components/GlobalSearch.tsx`: focus → nạp jobs (`getAdminJobPostings` HR / `getMyJobPostings` recruiter) + ứng viên (`applicationService.getApplications(mine)`), lọc client-side, dropdown 2 nhóm (icon Briefcase/User), bấm → mở thẳng chi tiết (`/{hr|recruiter}/jobs|my-jobs/:id`, `/{hr|recruiter}/candidates/:id`); Enter mở kết quả đầu (ưu tiên job); gộp ứng viên trùng email; cache 60s, đóng khi click ngoài/Esc. `HrLayout` nhúng trực tiếp (scope=hr); `WorkspaceLayout` thêm prop `globalSearchScope` (Recruiter=recruiter) — có thì render dropdown, không thì giữ ô đơn giản (Super Admin vẫn `?search=` → users). i18n `search.{jobs,candidates,noResults,loading}` (VI/EN) trong `modules/shared/nav`. Nút search icon mobile → mở trang danh sách ứng viên. Typecheck StaffSite pass.

- [x] 2026-08-09: **Unit test dời lịch phỏng vấn (test-plan B4) — 9 test mới, tổng 405/405 pass.** File `tests/ARI.Application.UnitTests/Scheduling/RescheduleBookingTests.cs` phủ `InterviewService.RescheduleBookingAsync`: guard booking lạ → "Không tìm thấy lịch phỏng vấn"; dời sang chính ca đang giữ → "đã nằm trong ca này"; ca đích thiếu → "Không tìm thấy ca phỏng vấn đích"; khác vòng → "thuộc vòng thi khác"; quá khứ → "đã diễn ra trong quá khứ"; đầy → "đã đầy" (guard fail không chốt chỗ). Happy path: nhả chỗ ca cũ (`Math.Max` floor 0) + chốt chỗ ca mới, booking trỏ ca mới + reset `Status="scheduled"`/`ConfirmationStatus="pending"`/xoá `DeclineReason`+`RespondedAt`, tạo bell `schedule_rescheduled` + realtime `InterviewRescheduled`; hồ sơ không có account → vẫn dời nhưng không notif. Tái dùng `SchedulingData`/`InMemoryUnitOfWork`/`RecordingNotificationService` (overload `Create(uow, notif)`). `dotnet test`: **405/405 pass**.

- [x] 2026-08-09: **Unit test sinh Evaluation buổi THẬT (test-plan B1) — 10 test mới, tổng 396/396 pass.** File `tests/ARI.Application.UnitTests/InterviewEvaluation/EvaluationGenerationTests.cs` phủ `InterviewService.EndSessionAsync → GenerateEvaluationReportAsync`: (ADR-053) AI chấm `not_pass` KHÔNG đổi `Application.Status` (giữ `interview`), lưu `AiVerdict/OverallScore/SessionType/RoundNumber`, `EvaluationCallCount==1`; (ADR-051) buổi thật báo `hr_admin` `ReceiveSystemEvent`/`AiEvaluationComplete`, buổi thử sinh đánh giá nhưng KHÔNG báo HR; (ADR-054) `CheatScore` theo trọng số từng loại (fullscreen_exit 8 / tab_hidden 12 / unknown 5 → 37; cap 100) + `CheatSignals` JSON gộp theo loại (hết ghi cứng `"[]"`); (ADR-051) `LanguageAssessment` null khi không có câu trả lời, có CEFR khi có; idempotent (phiên `completed`)/status `abandoned` bỏ sinh đánh giá; session lạ → fail. Tái dùng `InMemoryUnitOfWork`/`StubAiProvider`/`RecordingNotificationService`/`PracticeData` (không thêm mocking lib, không sửa hạ tầng chung). Kèm tài liệu `docs/test-plan.md` (356 case đã test + 131 case đề xuất). `dotnet test`: **396/396 pass**.

- [x] 2026-08-09: **Ô tìm kiếm trên header (mọi role) hoạt động thật thay vì trang trí.** Trước đây các ô search chỉ có placeholder (staff không handler; candidate chỉ `onFocus → navigate('/jobs')`). Nay controlled input + Enter → điều hướng trang đích kèm `?search=<từ khoá>`; đổi kbd `⌘K` → `⏎`. **Candidate** (`CandidateHeader.tsx` desktop+mobile) → `/jobs?search=`; `FindJobPage` thêm `useSearchParams`, khởi tạo `searchQuery` từ param + effect đồng bộ rồi dọn URL. **HR** (`HrLayout.tsx` standalone) → `/hr/candidates?search=` (CandidatesPage đã đọc `search` param sẵn). **Recruiter** → `/recruiter/candidates?search=` (đọc sẵn). **Super Admin** → `/super-admin/users?search=`; `WorkspaceLayout` thêm prop `searchPath` (Recruiter+SuperAdmin dùng chung), `UsersPage` thêm effect đọc `?search=` vào state rồi dọn URL. Nút search icon (mobile) cũng submit. Typecheck cả 2 site pass.

- [x] 2026-08-09: **Recruiter chỉ được sửa tin ở trạng thái `draft`/`rejected` — khoá khi `pending` (chờ HR duyệt) hoặc đã duyệt (`active`...).** 3 lớp: (1) FE `recruiter/JobDetailPage.tsx` đổi `canEdit` từ `status !== 'archived'` → `status === 'draft' || status === 'rejected'` (ẩn nút "Sửa tin"). (2) FE `CreateJobPostingPage.tsx` edit-mode load thêm guard: recruiter mở tin không phải draft/rejected → `loadError` chặn form + thông báo. (3) BE `UpdateJobCommand` thêm bước 2b: caller không phải SuperAdmin/HrAdmin và job không ở draft/rejected → `Result.Failure(..., Forbidden)` (đề phòng gọi API trực tiếp). Tests: sửa `Active_job_update_broadcasts_public_update` → role HrAdmin (recruiter không sửa được active), thêm `Recruiter_cannot_edit_after_submitted_or_approved` (pending/active/closed → Forbidden), `Recruiter_can_edit_draft_or_rejected`, `Admin_can_edit_pending_job`. Typecheck StaffSite pass; UpdateJobCommandHandlerTests 16/16 pass (Release).

- [x] 2026-08-09: **Ẩn nút "Ngân hàng câu hỏi" (Online Test Bank) khi job không có vòng trắc nghiệm.** Trước đây nút hiện vô điều kiện ở cả `recruiter/JobDetailPage.tsx` (Link `.../online-test`) và `hr/JobPostingDetailPage.tsx` (button `.../online-test`). Bọc điều kiện `job.roundConfigs?.some((r) => (r.roundType || '').toLowerCase() === 'online_test')` — job không cấu hình vòng `online_test` thì không hiện nút (ngân hàng câu hỏi chỉ có nghĩa khi có vòng trắc nghiệm). Typecheck StaffSite pass.

- [x] 2026-08-09: **Đổi màu dấu `*` bắt buộc trên form tạo/sửa tin sang đỏ.** `CreateJobPostingPage.tsx` — thêm component `RequiredStar` (`<span className="text-red-500 ml-0.5">*</span>`); gỡ `*` khỏi 3 chuỗi i18n `jobTitle`/`jobDescription`/`interviewMode` (VI+EN) và literal ` *` ở `workAddress`, thay bằng `<RequiredStar />` sau nhãn tại 4 label (Tiêu đề, Mô tả, Chế độ phỏng vấn, Địa chỉ cụ thể). Typecheck StaffSite pass.

- [x] 2026-08-09: **Giới hạn thời lượng mỗi vòng phỏng vấn (sơ loại/chuyên môn) tối đa 20 phút — TRỪ vòng trắc nghiệm.** `CreateJobPostingPage.tsx` — thêm hằng `MAX_ROUND_MINUTES = 20`; ô "Phút" thêm `max={20}`, clamp trong `onChange` (`Math.min(..., 20)`) và `onBlur` (>20 → 20, trống/<1 → 20); mặc định state khởi tạo + `addRound` đổi 30/45 → 20; payload `roundConfigs` khi submit cũng clamp `[1,20]` để job cũ >20 không lọt qua. Thêm hint `form.minutesMax` ("Tối đa 20 phút" / "Max 20 minutes") VI/EN dưới ô nhập. **Ngoại lệ `online_test`:** vòng trắc nghiệm cấu hình thời gian riêng ở mục "Bài thi trắc nghiệm" → biến `capped = roundType !== 'online_test'` bỏ trần (không `max`, không clamp, ẩn hint) — staff nhập thời lượng bao nhiêu cũng được; submit chỉ đảm bảo ≥1. Typecheck StaffSite pass.
